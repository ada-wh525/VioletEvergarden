...
在 `test/envelope-home-intro` 分支首次打开首页，会看到邮务抽屉滑开、信封从衬垫上取起并放到桌面，封面上的本地日期与时间持续更新；随后拆信刀划开封口，信纸抽出、放平并摊开。右上角可跳过；同一标签页会话中不会反复播放。想重新查看时打开 `/?opening=1`，手机和减少动态效果设置同样可用。内容和交互保持在原首页，加载失败不会被入场遮住。
