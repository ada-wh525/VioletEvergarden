...
在 `test/envelope-home-intro` 分支首次打开首页，会看到信封被拿起、封面上的本地日期与时间更新、拆信刀划开封口、信纸抽出并展开的入场。右上角可跳过；同一标签页会话中不会反复播放。想重新查看时打开 `/?opening=1`，手机和减少动态效果设置同样可用。内容和交互保持在原首页，加载失败不会被入场遮住。
