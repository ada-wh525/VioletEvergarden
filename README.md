...
在 `test/envelope-home-intro` 分支首次打开首页，会看到信封从深蓝色信件托盘上升起，封面上的本地日期与时间持续更新；随后拆信刀沿上沿缓缓留下锯齿状裁口，信纸抽出、摊开并放大，停留展示中文名字。右上角可跳过；同一标签页会话中不会反复播放。想重新查看时打开 `/?opening=1`，手机和减少动态效果设置同样可用。内容和交互保持在原首页，加载失败不会被入场遮住。
